...
export function assembler(str: string): MachineCodeType[] {
    ...
    codeArray = codeArray.map(l => l.replace(/;.*/, '').trim()).filter(l => l.length > 0);
    ...
    let machineCode: MachineCodeType[] = [];
    let labelTable: LabelTableType = {};
    const instructions = ["ADD", "AND", "NOT", "STR", "LDR", "JMP", "RET", "JSR", "LD", "LDI", "LEA", "ST", "STI", "BR", "BRn", "BRz", "BRp", "BRnz", "BRnp", "BRzp", "BRnzp", "ORG", "END", "HLT", "HALT"];
    
    codeArray.forEach((line, index) => {
        const labelMatch = line.match(/^(\w+),/);
        if (labelMatch) {
            const label = labelMatch[1];
            if (!instructions.includes(label.toUpperCase())) {
...
function processAdd(instruction: string, currentAddress: number): MachineCodeType {
    ...
    if (src2OrImm.startsWith("#") || src2OrImm.startsWith("x") || src2OrImm.startsWith("b")) {
        typeBit = "1";
...
function processAnd(instruction: string, currentAddress: number): MachineCodeType {
...
function processNot(instruction: string, currentAddress: number): MachineCodeType {
...
function processStr(instruction: string, currentAddress: number): MachineCodeType {
...
function processLdr(instruction: string, currentAddress: number): MachineCodeType {
...
function processJmp(instruction: string, currentAddress: number): MachineCodeType {
    const baseR = parseInt(instruction.slice(4).trim()[1]).toString(2).padStart(3, "0");
...
function processLd(instruction: string, currentAddress: number, labelAddressMap:LabelTableType): MachineCodeType {
...
function processLdi(instruction: string, currentAddress: number, labelAddressMap:LabelTableType): MachineCodeType {
...
function processLea(instruction: string, currentAddress: number, labelAddressMap: LabelTableType): MachineCodeType {
...
function processSt(instruction: string, currentAddress: number, labelAddressMap: LabelTableType): MachineCodeType {
...
function processSti(instruction: string, currentAddress: number, labelAddressMap: LabelTableType): MachineCodeType {
...
function processBr(instruction: string, currentAddress: number, labelAddressMap: LabelTableType): MachineCodeType {
...
function processJsr(instruction: string, currentAddress: number, labelAddressMap: LabelTableType): MachineCodeType {
    
        if(instruction.startsWith("JSRR")){
            const baseR = parseInt(instruction.slice(5).trim()[1]).toString(2).padStart(3, "0");
